...
本文档描绘的是**目标架构和长期规划**，当前代码实现尚处于 MVP 阶段。以下是"文档设计"与"代码现实"的关键差异：

| 设计目标 | 当前实现状态 | 备注 |
|----------|--------------|------|
| FAISS / Annoy 向量索引 | ❌ 未实现 | 当前是纯内存 JSON + O(N) 余弦相似度暴力搜索 |
| SQLite 元数据存储 | ❌ 未实现 | 元数据存在内存中，重启后从向量索引恢复 |
| Redis 缓存 | ❌ 未实现 | 仅有无过期机制的内存 map 缓存 |
| gRPC 服务间通信 | ❌ 未实现 | 使用简单 HTTP |
| fsnotify 文件自动监听 | ❌ 未实现 | 需要手动点击扫描 |
| 人脸识别 | ❌ 未实现 | 规划在 v1.1 |
| 前端 Gallery / Settings / 统计图表页面 | ❌ 未实现 | 当前只有 Home 和 Search 两个页面 |
| 语音输入 / WebP 缩略图 / Service Worker | ❌ 未实现 | 前端基础功能已有，进阶功能待开发 |
| 自然语言搜索（CLIP） | ✅ 已实现 | 语义搜索可用 |
| AI 对话接口 | ✅ 已实现 | ml-service 和 Go 后端已打通 |
| EXIF 解析 | ✅ 已实现 | 扫描时解析拍摄时间、相机型号、分辨率 |
| 扫描进度可视化 | ✅ 已实现 | 前端实时轮询显示进度条 |
| 搜索过滤（时间/标签） | ✅ 已实现 | 前端支持时间范围和多标签筛选 |
| 一键启动脚本 | ✅ 已实现 | `make dev` 可一键启三个服务 |

**结论**：先以本文档作为方向指引，但具体开发时应以代码和 `PLAN.md` 的阶段性目标为准，避免过度超前设计。
